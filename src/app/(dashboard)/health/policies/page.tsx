...
export default function HealthDashboardPage() {
  ...
  async function fetchPolicies() {
    ...
    const { data, error } = await supabase
      ...
      .select('*, customer:customer_id(full_name, phone_primary), insurer:insurer_id(name), tpa:tpa_id(name)')
      .eq('archived', false)
  ...
  const filteredPolicies = policies.filter(policy => {
    ...
    return (
      ...
      policy.plan_name?.toLowerCase().includes(search) ||
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Health Policies</h1>
          <p className="text-slate-500 mt-1">Manage health insurance for individuals and families.</p>
        </div>
        <div className="flex items-center gap-3">
          <Link 
            href="/renewals?category=health"
          ...
          <Link 
            href="/claims/new?category=health"
            className="flex items-center gap-2 bg-white border border-slate-200 hover:bg-slate-50 text-slate-600 px-4 py-2 rounded-xl font-bold text-sm transition-all shadow-sm"
          >
            Register Claim
          </Link>
          <Link 
            href="/health/new"
            className="flex items-center gap-2 bg-rose-600 hover:bg-rose-500 text-white px-6 py-2 rounded-xl font-bold text-sm transition-all shadow-lg shadow-rose-600/20"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-slate-500 text-sm font-medium">Total Lives Covered</p>
            <h3 className="text-xl font-bold text-slate-900">342</h3>
          </div>
        </div>
        <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-slate-500 text-sm font-medium">Sum Insured Managed</p>
            <h3 className="text-xl font-bold text-slate-900">₹ 8.5 Cr</h3>
          </div>
        </div>
        <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm flex items-center gap-4">
          ...
          <div>
            ...
            <h3 className="text-xl font-bold text-slate-900">{formatCurrency(2450000)}</h3>
          </div>
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead className="bg-slate-50">
              <tr>
                <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Policy & Plan</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center text-slate-500 font-medium">No health policies found matching "{searchTerm}".</td>
                </tr>
              ) : (
                  ...
                  <tr key={policy.id} className="hover:bg-slate-50/50 transition-colors group">
                    <td className="px-6 py-4">
                      <div className="flex flex-col">
                        ...
                        <p className="text-xs text-slate-500 mt-0.5">{policy.insurer?.name} • {policy.plan_name}</p>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-sm font-medium text-slate-700">{policy.members?.length || 0}</span>
                        <span className="text-xs text-slate-400 ml-1">({policy.floater_flag ? 'Floater' : 'Individual'})</span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
