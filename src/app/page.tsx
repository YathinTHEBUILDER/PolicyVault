...
export default function RootPage() {
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] flex flex-col items-center justify-center space-y-16 py-12 px-4">
      {/* Hero Header */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 w-full max-w-6xl animate-in fade-in slide-in-from-bottom-12 duration-1000 delay-200">
        {workspaces.map((ws) => (
          <Link 
            ...
            href={`/${ws.id}`}
            className={cn(
              "group relative bg-white p-8 md:p-10 rounded-[48px] md:rounded-[56px] border border-slate-100 shadow-sm transition-all duration-500 hover:shadow-2xl hover:-translate-y-4 flex flex-col",
              ws.shadow,
              ws.hover
            )}
          >
            {/* Domain Indicator */}
            <div className={cn(
              "w-16 h-16 md:w-20 md:h-20 rounded-[28px] md:rounded-[32px] flex items-center justify-center mb-8 transition-transform group-hover:scale-110 group-hover:rotate-3 duration-500",
              ws.accent
            )}>
              <ws.icon className="w-8 h-8 md:w-10 md:h-10" />
            </div>

            <h2 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tighter mb-4">{ws.title}</h2>
